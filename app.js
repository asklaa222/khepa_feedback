  ...
  app.innerHTML = `
    ...
    <form id="evaluationForm">
      <div class="evaluation-item">
        <label for="scriptKnowledge">대본 숙지:</label>
        ${createCircleRating("scriptKnowledge")}
        <textarea id="scriptKnowledgeNote" placeholder="이 항목에 대한 노트를 작성하세요">${selectedStudent.notes[`stage${currentStage}`]?.scriptKnowledge || ""}</textarea>
      </div>
      <div class="evaluation-item">
        <label for="basicKnowledge">기초지식:</label>
        ${createCircleRating("basicKnowledge")}
        <textarea id="basicKnowledgeNote" placeholder="이 항목에 대한 노트를 작성하세요">${selectedStudent.notes[`stage${currentStage}`]?.basicKnowledge || ""}</textarea>
      </div>
      <div class="evaluation-item">
        <label for="posture">올바른 자세 유지:</label>
        ${createCircleRating("posture")}
        <textarea id="postureNote" placeholder="이 항목에 대한 노트를 작성하세요">${selectedStudent.notes[`stage${currentStage}`]?.posture || ""}</textarea>
      </div>
      <div class="evaluation-item">
        <label for="nonVerbal">비언어적 표현 활용:</label>
        ${createCircleRating("nonVerbal")}
        <textarea id="nonVerbalNote" placeholder="이 항목에 대한 노트를 작성하세요">${selectedStudent.notes[`stage${currentStage}`]?.nonVerbal || ""}</textarea>
      </div>
      <div class="evaluation-item">
        <label for="pronunciation">발음:</label>
        ${createCircleRating("pronunciation")}
        <textarea id="pronunciationNote" placeholder="이 항목에 대한 노트를 작성하세요">${selectedStudent.notes[`stage${currentStage}`]?.pronunciation || ""}</textarea>
      </div>
      <div class="evaluation-item">
        <label for="breathing">호흡:</label>
        ${createCircleRating("breathing")}
        <textarea id="breathingNote" placeholder="이 항목에 대한 노트를 작성하세요">${selectedStudent.notes[`stage${currentStage}`]?.breathing || ""}</textarea>
      </div>
      <div class="evaluation-item">
        <label for="speed">진행속도:</label>
        ${createCircleRating("speed")}
        <textarea id="speedNote" placeholder="이 항목에 대한 노트를 작성하세요">${selectedStudent.notes[`stage${currentStage}`]?.speed || ""}</textarea>
      </div>
      <div class="evaluation-item">
        <label for="interaction">Interaction:</label>
        ${createCircleRating("interaction")}
        <textarea id="interactionNote" placeholder="이 항목에 대한 노트를 작성하세요">${selectedStudent.notes[`stage${currentStage}`]?.interaction || ""}</textarea>
      </div>
      <div class="evaluation-item">
        <label for="voiceTone">목소리의 높낮이:</label>
        ${createCircleRating("voiceTone")}
        <textarea id="voiceToneNote" placeholder="이 항목에 대한 노트를 작성하세요">${selectedStudent.notes[`stage${currentStage}`]?.voiceTone || ""}</textarea>
      </div>
      <div class="evaluation-item">
        <label for="movementUnderstanding">장소의 이동방법 이해도:</label>
        ${createCircleRating("movementUnderstanding")}
        <textarea id="movementUnderstandingNote" placeholder="이 항목에 대한 노트를 작성하세요">${selectedStudent.notes[`stage${currentStage}`]?.movementUnderstanding || ""}</textarea>
      </div>
...
function showFinalEvaluation(index) {
  ...
  Object.keys(student.scores).forEach(stage => {
    ...
    Object.keys(stageScores).forEach(id => {
      ...
      summary += `<li>${getStageLabel(id)}: ${percentile}% - ${message}</li>`;
   // 각 항목에 대한 노트를 불러오기
...
function getStageLabel(id) {
  const labels = {
    ...
    voiceTone: "목소리의 높낮이",
    movementUnderstanding: "장소의 이동방법 이해도"
  };
