...
function saveEvaluation() {
  // Save the stage name for the current stage
  const stageNameInput = document.getElementById('stageName');
  selectedStudent.stageNames[currentStage - 1] = stageNameInput.value;

  // All scores are already stored in `selectedStudent.scores`, so nothing more needs to be done here.
}
...
function nextStage() {
  ...
  if (currentStage < 3) {
    ...
    renderStudentList(); // After the last stage, return to the student list
  }
}
...
function showFinalEvaluation(index) {
    ...
    let summary = `[실습] 경복궁 문화유산해설사 1회차 실습평가 피드백:<h3>이름: ${student.name}</h3>`;
    
    // 평균 점수 추가
    summary += `<p><strong>점수:</strong> ${averagePercentile}점</p><br>`; // 점수 아래 줄 바꿈 추가
    ...
    Object.keys(student.scores).forEach(stage => {
      ...
      Object.keys(stageScores).forEach(id => {
        ...
        const message = getScoreMessage(percentile); // 점수에 맞는 문구 추가
    ...
    // Display the summary in the app element
    app.innerHTML = `
      <h2>최종 평가</h2>
      ${summary}
...
function getStageLabel(id) {
  ...
  return labels[id] || id; // 만약 레이블이 없으면 id 그대로 반환
}
